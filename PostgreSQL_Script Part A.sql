--Created the schema called assignment within my newly created database; lux_assignment
...
-- create table customers table in the schema assignment
...
-- Create another table called products in the schema assignment 
...
--Created Sales table in the assignment schema
...
--Create inventory tables in the assignment schema
...
-- Insert data into assignment.Products table
...
-- Insert data into assignment.sales table
insert into assignment.sales 
(sales_id, customer_id, product_id, quantity_sold, sale_date, total_amount)
values
...
-- View all the tables within the schema that was created
select table_name from information_schema.tables 
where table_schema = 'assignment';
...
select count(p.product_id) as total_count_of_products from assignment.products p;
...
-- Assigned alias to the result as average_product_price
select AVG(p.price) 
...
-- Assigned alias to the result as Sales_Revenue
...
-- Assigned alias to the quesry result as membership_status_summary
select distinct c.membership_status 
	as membership_status_summary
...
-- Assigned column name to the result as electronic_products 
...
-- Assigned column name to the result as highest_price
select max(p.price) 
as highest_price
from assignment.products p;

-- 10. Write a query to count the number of sales for each product from the `Sales` table.
--Created the first query that helps count the occurence of each product_id
	-- The query additionally product ids were arranged in a chronological order
select s.product_id,count(s.sales_id)
	as sale_count
from assignment.sales s
...
--Used the select column to retrieve the data
select p.product_name, 
	SUM(s.quantity_sold) 
		as total_units_sold 
from assignment.products p
...
select MIN(p.price) as lowest_price
from assignment.products p;


-- 13. Write a query to find customers who have purchased products with a price greater than 1000.

--innerjoin to show the customer names and the sales amounts they purchased
select s.customer_id, c.first_name,c.last_name,s.total_amount, c.email, c.membership_status
from assignment.customers c 
inner join assignment.sales s 
on s.customer_id = c.customer_id
where s.total_amount > 1000;

--Cross-checked the customer ids of the customers who made purchases greater than 1000
select s.customer_id as expensive_sales
from assignment.sales s 
where s.total_amount > 1000;
...
--This shows all customers even those who made no purchaese; with null values coming up as last
select c.customer_id, c.first_name, c.last_name, 
SUM(s.total_amount) as total_customer_spend
from assignment.customers c 
...
select 
c1.first_name as first_name_A,
c1.last_name as last_name_A,
c2.first_name as first_name_B,
c2.last_name as last_name_B,
...
select s.product_id, 
	...
	SUM(s.total_amount) as total_revenue
		from assignment.sales s 
		group by s.product_id )
select p.product_name, p.category, unit_sales.total_sales, unit_sales.total_revenue
from unit_sales 
join assignment.products p 
on unit_sales.product_id = p.product_id
where total_sales > 5;
...
group by p.product_name, p.category, rev.total_revenue;
...
select s.customer_id, c.first_name, c.last_name, s.quantity_sold,
...
--This limits the results of the 2023 sales to the top 5 highest sales:
...
on s.customer_id = c.customer_id 
where extract (year from s.sale_date) = 2023
group by c.customer_id, c.first_name, c.last_name, extract (year from s.sale_date) 
...
select
	p.product_name, 
	max(s.total_amount/s.quantity_sold) as highest_price
		from  assignment.sales s
join assignment.products p 
on p.product_id = s.product_id
group by p.product_name
...
select s.customer_id , sum(s.total_amount) as sales 
from assignment.sales s
group by s.customer_id)
select count(*) from sales_amount 
where sales_amount.sales > 500;
...
select 
count(s.sales_id) as total_number_of_sales,
c.membership_status
from assignment.sales s 
join assignment.customers c 
on c.customer_id = s.customer_id
...
--using the group by function to solve the query:
...
--using the winder functions over and partition by to solve the query:
select * from (
select c.first_name,c.last_name,
	SUM (s.quantity_sold) OVER(partition by c.customer_id ) as units_sold
from assignment.sales s 
join assignment.customers c 
on c.customer_id = s.customer_id) 
where units_sold >5;
...
select 
	p.product_name, 
	p.category, 
	AVG(s.quantity_sold) as average_sales
from assignment.sales s 
inner join assignment.products p
on p.product_id = s.product_id
...
select count(Dec_2023_sales.sales_id) as Count_of_Dec2023_Sales from Dec_2023_sales;


-- 32. Write a query to find the total amount spent by each customer in 2023 and list the customers in descending order.

--The only limitation with this query is that it doesn't collate the data by customer_id
-- This means that the could be duplicates below 
...
--To correct this, one should use the query below collating the totals for the sales amount and units sold
select c.first_name,
		c.last_name,
...
--The CTE labelled as first_purchase_date derives the first date the customer by id made the purchase
--This is helpful for sales data by customer across multiple days (the reason for the group by clause)
--The where clause in this select query removes any customer who made a purchase first then registered to the program
--Basically removes the negatives from the days in between
...
first_purchase_date.first_purchase - c.registration_date as days_between_purchases
from assignment.customers c 
join first_purchase_date 
on c.customer_id = first_purchase_date.customer_id
where first_purchase_date.first_purchase - c.registration_date between 0 and 7;
...
--This CTE is meant to calculate the number of times a customer made a purchase
--In this case, it is best to not have a limit to ensure full output of results
with sales_count as (
select s.customer_id,
count(s.sales_id) as sale_per_customer
from assignment.sales s
group by s.customer_id)
select 
c.first_name,
c.last_name,
max(sales_count.sale_per_customer) as no_of_purchases
from assignment.customers c 
join sales_count 
on sales_count.customer_id = c.customer_id
group by c.customer_id;

-- To ensure that only 1 value is returned, then the below works best
select 
c.first_name,
c.last_name,
count(s.sales_id) as sale_per_customer
...
select MIN(i.stock_quantity) as lowest_stock,
MAX(i.stock_quantity ) as highest_stock
...
select 
	...
	c.last_name,
	p.product_name,
	SUM(s.quantity_sold) * p.price as product_sales
from assignment.products p 
inner join assignment.sales s
on s.product_id = p.product_id 
join assignment.customers c 
on s.customer_id = c.customer_id
where c.membership_status = 'Gold'
group by c.first_name, c.last_name, p.product_id, p.product_name;


-- 42. Write a query to find the total sales of products by category.
-- The CTE on this formula will help return the summation of the total sales by product_id sold
-- The select statement after the CTE returns the revenue by category
  
with total_sales as (
select s.product_id, SUM(s.total_amount) as total_sales from assignment.sales s 
group by s.product_id)
select p.category, SUM(total_sales) as revenue_by_category
from assignment.products p
join total_sales
on p.product_id = total_sales.product_id 
group by p.category
...
join assignment.sales s
on s.product_id = p.product_id 
group by p.product_name, year_of_sale, sale_month
...
--The having clause filters out products by the set quantity
-- The 2 joins are necessary to add the product_names to the inventory
-- The first sum aggregate function helps to get a total of the quantities purchased per product_id(different days)
select i.product_id, 
	p.product_name,
	i.stock_quantity,
	SUM(s.quantity_sold) as units_sold,
...
--Grouped by customer_id because this is unique and it helsp with the grouping of the aggregate funstion; sum
--Order by the total sales in desc order captured the highest_values first
--Limit clause returns the 5 values
...
select count(distinct s.product_id) as total_unique_products_sold
from assignment.products p 
join assignment.sales s 
on s.product_id = p.product_id
where extract (year from s.sale_date) = 2023;
...
--This formula takes into acount products bought more than once (hence the max within ths sale date to get the most recent purchase)
-- The current_date format is to keep the formula running as the dates change
-- The 6*31 is used to calculate the number of days in the 6 mths(assumption is that each month has 31 days)
select p.product_name,
p.category,
p.supplier,
p.stock_quantity,
...
join assignment.sales s 
on p.product_id =s.product_id 
where s.sale_date > current_date - (6*31)
group by p.product_name, p.category, p.supplier, p.stock_quantity
order by p.category;
...
select 
	...
	p.price,
	SUM(s.quantity_sold) as total_units_sold,
	p.price * SUM(s.quantity_sold) as product_revenue
from assignment.products p 
join assignment.sales s
	on s.product_id = p.product_id
...
--This CTE actually divides the customers who made purchases into 3 groups; high, medium and low spenders
with spenders as (
select 
	s.customer_id,
	s.product_id,
	s.sale_date,
	SUM(s.total_amount) as Total_sales,
	ntile(3) over(order by SUM(s.total_amount) DESC) as spending_ranks
from assignment.sales s 
where extract (year from s.sale_date) = 2023
group by  )
select 
...
c.last_name,
spenders.sale_date, 
spenders.total_amount
from spenders 
join assignment.customers c 
on spenders.customer_id = c.customer_id 
where spending_ranks = 1;

-- To get the customer(s) with only the highest spend
select 
	s.product_id,
	s.sale_date,
	s.total_amount,
...
(select
	s.product_id,
	count(s.sales_id) as sale_by_product
from assignment.sales s
group by s.product_id)
select p.product_name,
p.price,
sale_count.sale_by_product 
from sale_count
join assignment.products p 
on p.product_id = sale_count.product_id 
where p.price >200 
and sale_count.sale_by_product >100;
